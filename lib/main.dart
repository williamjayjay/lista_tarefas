...
class _HomeState extends State<Home > {
      ...
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            ...
            child: RefreshIndicator(onRefresh: _refreshNow,
              child: ListView.builder(
                  padding: EdgeInsets.only(top: 10.0),
                  itemCount: _toDoList.length,
                  itemBuilder: buildTheItem),),
          )
        ],
      ) ,
    );
  ...
  Widget buildTheItem(BuildContext context, int index){  //função para buildar intens.
    return Dismissible(
  ...
  Future<File> _saveData() async {  //salvar dados
    String data = json.encode(_toDoList);
    final file = await _getFile();
    return file.writeAsString(data);
  }

  Future<String> _readData() async { //recuperar dados
    try {
      final file = await _getFile();
